@t2v_deeplink_1.1_tag
Feature: T2V Deeplink Integration Tests

Background:
  * configure headers = req_headers
  * def payloads = read('classpath:config/data/payloads/t2v_deeplink/t2v_deeplink_1.1_payload.yml')
  * def responses = read('classpath:config/data/responses/t2v_deeplink/t2v_deeplink_1.1_response.yml')
...
@t2v_deeplink_1.1_tag8
Scenario: Unhappy Path 3 - verificationStatus != PROCESSED, appendCookie: FAILURE, cookie?: Yes, redirectUrl: failureUrl
  # Call Step 1
  * def step1Response = call wfResponse { payload: '#(payloads.t2v_deeplink_happy_or_unhappy_path_bank_step_1_req)', response: '#(responses.default_step1_success_resp)', httpStatus: 200 }
  # Sleep for 3 seconds
  * sleep(3000)
  # Set URL for Step 2
  * def baseURL = URL.slice(0, URL.indexOf('initiate-policy'))
  * def URL = baseURL + 'policy-processes/' + step1Response.response.metadata.policyProcessId + '/continue-process'
  # Call Step 2
  * call wfResponse { payload: {}, response: '#(responses.bank_policy_failure)', httpStatus: 200 }
...
#  out the @t2v_deeplink_1.1_tag9 tag
#@ignore
@t2v_deeplink_1.1_tag9
...
# NOTE: this scenario is not intended to run against local. If attempting to run this entire feature file against local, uncomment the @ignore tag and comment
#  out the @t2v_deeplink_1.1_tag10 tag
#@ignore
@t2v_deeplink_1.1_tag10
