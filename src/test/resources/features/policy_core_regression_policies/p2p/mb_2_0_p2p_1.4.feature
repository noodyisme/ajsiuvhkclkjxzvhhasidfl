...
Feature: P2P Component Tests
  ...
  Background:
    ...
    * def URL = URL + '?policyName=mb_2_0_p2p_a&policyVersion=1.4'

  Scenario: Validate Masterbuilder API can successfully load a policy which is using P2P Component along with Gateway Policy call in chain
    * call wfResponse { payload: '#(payloads.mb_2_0_p2p_req)', response: '#(responses.mb_2_0_p2p_1dot4_resp)', httpStatus: 200 }
